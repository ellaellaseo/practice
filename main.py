...
#TODO Test different version with a device.
# = Tried 2.2.0 and 2.2.1, both passed successfully.

#TODO Test solution bank changes with a device > create a Pull Request for Tony to review.
# = Tested the solution bank function with a device. Can see the solution at the end of the test.

#TODO NEXT TIME
# Changing between zeus and janus.
...
logging.setLogRecordFactory(record_factory)
    ...
    #TODO
# This is a recommended solutions for the issues that occur commonly during the Test
def quick_solution(solution):
    solution_bank = {
        'firmware upgrade': 'Quick Solution : Check if ethernet cable / 10 pin cable is stable',
        'led': 'Quick Solution : This is an LED Component issue of the OEM Board. Bring it to hardware team to fix it',
        'i2c readings': 'Quick Solution : Check if the SoM and OEM Board are connected properly and try again',
        'Getting IP': 'Quick Solution : Check if the Y cable is stable',
        'DUT Power during streaming': 'Quick Solution : Check if ethernet cable / 10 pin cable is stable',
        'FPS and resolution check': "Quick Solution : This issue occurs when it doesn't reach the target FPS. Try to rerun the test once or twice",
        'recording': 'Quick Solution : Check if the Y cable is stable and try again',
    }
    return LOGGER.info(solution_bank[solution])
...
file_handler = logging.handlers.WatchedFileHandler(f'{socket.gethostname()}_device_log', mode='a')
...
def run():
    ...
    LOGGER.info('Starting tests for Deeptrekker!\n\n\n')
    ...
    try:
        ...
        assert device.oclea_info['dts-model'].strip() == 'Oclea S5L Janus'
    ...
    i2c_result = True
    expected = [5, 0, 2]
    for idx, addr_count in enumerate(expected):
        out = device.send_command(f'i2cdetect -y {idx}')
        LOGGER.info(out)
        if out.count('UU') != addr_count:
            i2c_result = False
        if idx == 2 and out.count('41') != 1:
            i2c_result = False
    ...
    duration = 120
    slack = 20
    fn = f'dt_{SERIAL_NUMBER}.mp4'
    blackdetect_fn = f'dt_{SERIAL_NUMBER}_blackdetect.mp4'
    ...
    try:
        ...
        if not ip:
            ...
            device.send_command(f'oclea_bootstrap {device.sensor_name}')
            ...
            # Choose 4.3 because 4.25 was the max of ~100 units processed in
            # Oct 2022
            if avg_power > 4.3:
